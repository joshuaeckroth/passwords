...
64 rules, top 10,000 rules, top 50,000, etc.
...
\item OneRuleToRuleThemAll with our generated rules
appended, then trimmed to the size of the `dive' ruleset (99k rules), which we
refer to as \textit{OneRuleToFindThem} (ORTFT).
\item Pantagrule's `one.royce' rule list\cite{pantagrule}. Pantagrule rules are
generated by running PACK on a private massive set of plaintext passwords.
Since we do not have access to this set, we trained PACK on RockYou and call
this list of rules PACK, mentioned above. Pantagrule's `one.royce' rule set is
the top 100k rules generated by their procedure.
\item Pantagrule's top-performing and largest rule list,
...
OneRuleToRullThemAll, dive, and pantagrule.private.v5.popular.
...
\begin{tabular}{|l|l|l|l|}
  ...
  ORTFT (ORTRTA + Ours) & 99,092 & X & X \\
  Pantagrule `one.royce' & 99,092 & 71.93\% & X \\
