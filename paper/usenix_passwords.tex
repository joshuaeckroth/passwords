...
Research into the efficacy of variations on this popular method of cracking
is merited primarily on the grounds that it can inform good password policy.
Many organizations attempt to enforce some level of password strength through
length and character diversity requirements that may provide a false sense
of security. If a user meets these requirements by appending `123!' to a
weak password but a rule that applies this modification is known to be effective
and widely incorporated in common rule sets, it is possible or even likely that these extra
characters will not provide significant protection to a user. One
can imagine using information about what rules are effective against existing
real world passwords to prevent users choosing passwords that are weak to them.
