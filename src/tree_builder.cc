...
TreeBuilder::TreeBuilder(const vector<string> *target_passwords, const vector<string> *dict_words, set<string> &rules, int target_cnt, float score_decay_factor, size_t max_cycles)
    : rules(std::move(rules)), choose_pw_cnt(target_cnt), score_decay_factor(score_decay_factor), max_cycles(max_cycles) {
