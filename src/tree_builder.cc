...
// If a new password pw generated by a rule has identical intermediate passwords produced by
// intermediate rule applications, strip extraneous rules and return new composite rule
std::string TreeBuilder::strip_intermediate(unsigned int orig_target_idx, string rule, const char *pw) const {
    string new_rule = rule;
    const size_t rule_size = rule.size();
    string password = (*(this->targets))[orig_target_idx];
    ...
    // $1 $2 [ ] [ ]
    //   2  5 7 9 11
    // {pw1, 2} {pw2, 5} {pw3, 7} {pw4, 9} {pw5, 11}
    //
    vector<pair<string, size_t>> intermediate_pws;
    intermediate_pws.reserve(rule_size / 2);
    for (size_t idx = 0; idx < space_indices.size(); idx++) {
        const size_t si = space_indices[idx];
        char *new_pw = this->apply_rule(rule.substr(0, si), password.c_str());
        string s(new_pw);
        free(new_pw);
        intermediate_pws.push_back({s, si});
    }
    size_t end = intermediate_pws.size();
    for (size_t idx = 0; idx < end; idx++) {
        const string pw1 = intermediate_pws[idx].first;
        const size_t ri1 = intermediate_pws[idx].second;
        for (size_t j = intermediate_pws.size(); j > idx; j--) {
           const string pw2 = intermediate_pws[j].first;
           const size_t ri2 = intermediate_pws[idx].second;
           const bool is_intermediate = pw1 == pw2;
           if (is_intermediate) {
               vector<pair<string, size_t>> v1 = {intermediate_pws.begin(), intermediate_pws.end()};
               vector<pair<string, size_t>> v2 = {intermediate_pws.begin(), intermediate_pws.end()};
               v1.insert(v1.end(), v2.begin(), v2.end());
               intermediate_pws = v1;
               string rs1 = new_rule.substr(0, ri1);
               string rs2 = new_rule.substr(ri2);
               const size_t size_diff = ri2 - ri1;
               for (auto &p : intermediate_pws) {
                   if (p.second > ri1) {
                       p.second -= size_diff;
                   }
               }
