...
int main(int argc, const char** argv) {
    ...
    // CREATE PRIORITY QUEUE OF RULES
    vector<Rule> rule_vec = RuleLoader::load_rules(argv[2]);
    std::priority_queue<Rule, vector<Rule>, std::less<Rule>> rule_q(rule_vec.begin(), rule_vec.end());
    print_queue(rule_q);
