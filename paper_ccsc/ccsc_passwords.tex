...
\addbibresource{ccsc_passwords.bib}

\title{OneRuleToFindThem: Efficient Automated Generation of Password Cracking Rules\footnote{\protect\input{copyright}}
}
...
\begin{figure}[ht!]
\includegraphics[width=\linewidth]{figures/example-pw-rules.pdf}
...
this way, but we can use the Pwned Hashes list\autocite{pwned}, which includes
frequencies.
...
best-in-class for large rule sets, we believe that in nearly matching
a best-in-class approach that we have demonstrated both the efficacy of our
...
Pantagrule's `pantagrule.private.v5.popular.rule'~\autocite{pantagrule}. The `Count'
...
We have not exhausted all possibilities for strong algorithms or rule-generation
heuristics in the area of `rule-first' rule generation. For example, one could
imagine taking our approach but boosting rule scores only if a rule produces
a stronger password from a weaker one, possibly eliminating some rules (like
many including `remove last character') that we currently score higher than
their experimental results indicate is appropriate.

In order to get a better picture of RPP trends, one of the more important metrics
